...
export function ServiceLocationPage({ service, location, slug }: ServiceLocationPageProps) {
  ...
  const pageUrl = `${BUSINESS.website}/${slug}`
  const schemas = [
    ...
    generateWebPageSchema(
      `${service.name} in ${location.name} | 24/7 | ${location.responseTime} Response`,
      `${service.name} in ${location.name}. ${location.responseTime} response. No call-out fee.`,
      pageUrl
    ),
    generateHowToSchema(
      `How Our ${service.name} Service Works in ${location.name}`,
      `Step-by-step process for ${service.name.toLowerCase()} in ${location.name}.`,
