  ...
  if (detected.type === 'service-location' && detected.serviceSlug && detected.locationSlug) {
    ...
    if (service && location) {
      return {
        ...
        description: `${service.name} in ${location.name}, ${location.borough}. ${location.responseTime} response, available 24/7. No call-out fee. Fully insured. Call 020 3900 4444.`,
        keywords: [
        ...
        alternates: { canonical: `https://londonlocksmithpro.co.uk/${slug}` },
        openGraph: {
          title: `${service.name} ${location.name} | 24/7 | ${location.responseTime} Response`,
          description: `${service.name} in ${location.name}. ${location.responseTime} response. No call-out fee.`,
          url: `https://londonlocksmithpro.co.uk/${slug}`,
          type: 'website',
        },
      }
    }
