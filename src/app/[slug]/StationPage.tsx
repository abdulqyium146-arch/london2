...
import { Phone, Train, Clock, CheckCircle2, ChevronRight, MapPin, Shield, ArrowRight } from 'lucide-react'
...
export function StationPage({ station, slug }: StationPageProps) {
  const stationFaqs: FAQ[] = [
    {
      ...
      answer: `Yes. London Locksmith has locksmiths covering the ${station.name} area and surrounding ${station.area} streets. If you're locked out near ${station.name} station, call ${BUSINESS.phone} and we'll have someone with you in approximately ${station.responseTime}.`,
    },
    {
  ...
  // Nearby stations (all except current)
  const nearbyStations = stations.filter((s) => s.slug !== station.slug).slice(0, 8)

  // Postcode for this station's location
  const stationPostcodes = postcodes.filter((p) => p.locationSlug === station.locationSlug)
          ...
          <div className="lg:col-span-2 space-y-12">
            ...
            <div>
              <h3 className="text-2xl font-bold text-white mb-5">
                Emergency Locksmith Near {station.name}
              </h3>
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs text-slate-500 mt-0.5">
                        From £{service.priceFrom} · {station.responseTime}
                      </div>
                    </div>
            ...
            <div>
              <h3 className="text-2xl font-bold text-white mb-5">
                All Locksmith Services near {station.name}
              </h3>
            ...
            {station.landmarks.length > 0 && (
              <div className="flex items-start gap-3 bg-[#111827] border border-gray-800 rounded-xl p-4">
          ...
          <div className="space-y-5">
            ...
            <div className="bg-gradient-to-br from-orange-600/20 to-red-600/20 border border-orange-500/30 rounded-2xl p-6">
              <h3 className="text-white font-bold text-lg mb-3">
                Emergency Near {station.name}?
              </h3>
              <p className="text-slate-300 text-sm mb-4">
                {station.responseTime} response. Available 24/7.
              </p>
              <Link
                href={BUSINESS.phoneHref}
