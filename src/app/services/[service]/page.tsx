...
export default async function ServicePage({ params }: Props) {
  ...
  const serviceUrl = `${BUSINESS.website}/services/${service.slug}`
  const schemas = [
    generateServiceSchema(service),
    generateFAQSchema(service.faqs),
    generateBreadcrumbSchema([{ name: 'Home', href: '/' }, ...breadcrumbs]),
    generateWebPageSchema(service.metaTitle, service.metaDescription, serviceUrl, 'WebPage'),
    generateHowToSchema(
      `How Our ${service.name} Service Works`,
      `Step-by-step process for ${service.name.toLowerCase()} in London.`,
